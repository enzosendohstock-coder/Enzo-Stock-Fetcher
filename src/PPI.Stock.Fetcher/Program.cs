...
if (args.Length > 0 && args[0] == "--auto-backfill-monthly-revenue")
{
    var autoBackfillWorkerSettings = config.GetSection("WorkerApi").Get<WorkerApiSettings>()
        ?? throw new InvalidOperationException("找不到 WorkerApi 設定區段，請檢查 appsettings.json。");
    using var autoBackfillHttpClient = new HttpClient();
    var autoBackfillApi = new WorkerApiClient(autoBackfillHttpClient, autoBackfillWorkerSettings);
    var autoBackfillWatchlist = await autoBackfillApi.GetWatchlistAsync();
    var autoBackfillMonthlyRevenue = new MonthlyRevenueClient(autoBackfillHttpClient);
...
static async Task AutoBackfillMonthlyRevenueAsync(
    List<string> watchlist,
    WorkerApiClient api,
    MonthlyRevenueClient monthlyRevenue,
    MonthlyRevenueHistoryClient historyClient)
{
    ...
    foreach (var stockCode in stocksNeedingBackfill)
    {
        try
        {
            await BackfillMonthlyRevenueHistoryAsync(stockCode, deepStart, api, monthlyRevenue, historyClient);
        }
        catch (Exception ex)
        {
            Console.WriteLine($"警告：{stockCode} 月營收自動回補失敗，跳過並繼續下一支。錯誤：{ex.Message}");
        }

        // 逐股票交接處額外間隔一下(單一股票內部逐月查詢本身已經有 800ms 間隔)，
        // 避免整批股票排隊回補時對 MOPS 形成持續高密度的請求。
        await Task.Delay(2000);
    }
